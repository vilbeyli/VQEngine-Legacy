...
:: Set xcopy parameters for generating scene files
...
:: src: https://stackoverflow.com/questions/60034/how-can-you-find-and-replace-text-in-a-file-using-the-windows-command-line-envir
:: Generate scene files, replace Template name with scene file name
...
:: src: https://stackoverflow.com/questions/7005951/batch-file-find-if-substring-is-in-string-not-in-a-file
:: src: https://stackoverflow.com/questions/23525681/how-to-print-variable-inside-loop-in-batch-script
:: Edit solution files and add the new scene files to the project file
...
@echo off
:: find the lines with CLInclude and CLCompile (=prevLine), which is also followed by </ItemGroup>
:: </ItemGroup> as current line and CLInclude/Compile as prevLine, we output to the new solution file
set prevLine=
for /f "tokens=* delims=," %%a in (%solutionFile%) do (
  ...
  if not "x!line:%currSearchStr%=!"=="x!line!" (
       if not "x!prevLine:%prevSearchStrCompile%=!"=="x!prevLine!" (
           :: !line! currently has </ItemGroup>, !prevLine! currently has the last cpp entry
           echo     ^<%compileXML%^>>> %updatedSolutionOutDir%
       )
       if not "x!prevLine:%prevSearchStrInclude%=!"=="x!prevLine!" (
           :: !line! currently has </ItemGroup>, !prevLine! currently has the last include entry
